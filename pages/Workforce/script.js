...
document.addEventListener("DOMContentLoaded", (eQ) => {
  ...
  function slide(direction) {
    ...
    if (direction === slide_r) count++;
    else if (direction === slide_l) count--;

    let item = items[count];

    active_label = document.querySelector(
      `.carousel-labelQ[for="${item.name}"]`
    );
...
document.addEventListener("DOMContentLoaded", (e) => {
  const store = {
    "Forbidden City":
      'They (net)work their way through everything that they need for Malhar and did the "Am I audible?" with an audience before it was the new normal. Supreme graphics, expert salesman shop and technical expertise is what they bring to the table!',
    "Jingshan Park":
      'Computers as a department chooses to reside on Discord. In addition to the painful tracking down of bugs and fixing them, you\'ll probably find one member of the department or another awake at any time of the day. Finding snippets of code through infinite hours of scrolling, clear out something thats "bugging" you, or helping the other OGs with all the problems in their computers. All these fall right under their domain.',
    "Temple of Heaven":
...
document.addEventListener("DOMContentLoaded", (e2) => {
  const store2 = {
    "Forbidden City":
      "These are departments forming the compass that gives the fest direction.What are Management Departments like, you ask? Well they do the tech, the money, the budget AND they're funny.",
    ...
    "Beihai Park":
      "All venues and setup-related work are handled by logistics. It involves moving bulky items, setting up backgrounds, building and displaying stages and setting up benches and tables in Classrooms. Three auxiliary departments are present.Construction, Displays & Classrooms",
    "Houhai Park":
...
document.addEventListener("DOMContentLoaded", (e3) => {
  const store3 = {
    ...
    "Jingshan Park":
      "From the grace of contemporary to the swag of hip-hop, from the melodies of acapella to the beats of jazz, we've got it all covered! WPA is the platform that gives your passion wings!",
    "Temple of Heaven":
